...
clean_train_dataset = datasets.CIFAR100(
...
ADV_TRAIN_DIR = './adversarial_examples/GTSRB/training/'

# label
classes = os.listdir(ADV_TRAIN_DIR + '0.1/')

adv_train_list = []

for _class in classes:
    train_images = os.listdir(ADV_TRAIN_DIR + '0.1/' + _class + '/')
    for image in tqdm(train_images):
        adv_train_list.append(ADV_TRAIN_DIR + '0.1/' + _class + '/' + image)
...
NETWORK = 'vgg16'
DATASET = 'adversary_0.1_8.2_GTSRB_acc' # (epsilon)_(max-rate)_(dataset-name)
SAVE_DIR = './models/'
NUM_CLASSES = 43 # 10, 100, 43(GTSRB)
...
for epoch in range(EPOCHS):
    ...
    print('%d epoch acc : %.2f' % (epoch + 1, 100 * correct / total))
    logs += '%d epoch loss : %.3f' % (epoch + 1,
                                        running_loss / len(mixed_train_loader)) + '\n'
    logs += '%d epoch acc : %.2f' % (epoch + 1, 100 * correct / total) + '\n'
    logs += '=' * 25 + '\n'
